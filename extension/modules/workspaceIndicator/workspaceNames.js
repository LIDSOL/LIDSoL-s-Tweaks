...
export class WorkspaceNames {
    ...
    static init(workspaces) {
        WorkspaceNames._instance = new WorkspaceNames(workspaces);
        return WorkspaceNames._instance;
    }
    ...
    reorder(reorderMap) {
        ...
        for (const [newIndex, oldIndex] of reorderMap.entries())
            newWorkspaceNames[newIndex] = oldIndex >= 0 ? (oldWorkspaceNames[oldIndex] ?? '') : '';
        ...
        for (const windowName of windowNames) {
            workspacesNamesMap[windowName] = [
                ...(workspacesNamesMap[windowName] ?? []).filter(
                    (name) => name !== newName && this._getEnabledWorkspaceNames().includes(name),
                ),
                newName,
            ];
        }
