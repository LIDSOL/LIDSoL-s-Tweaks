...
- [ ] Fix, bug de posicionamiento en X. El menu box aparece hasta el extemo izquierdo de la pantalla, debería aparecer en el sitio en el cual se encuentran los quickSettings (más ademante se añadirá la posibilidad de mover los elementos del panel, así que debe aparecer en el lugar adecuado). Se hizo esto pero no se solucionó el bug:
  - El overlay perdía su constraint horizontal al deshabilitar las del contendor, apareciendo en $x$=0
  - /Clutter.BindConstraint/ para X vinculado a /menu-box/ (QuickSettings). Esto tenía la intención de mantener el overlay alineacio horizontalmente con los ajuster rápidos.
